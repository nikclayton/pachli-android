...
class Init : CliktCommand() {
    ...
    override fun run() = runBlocking {
        ...
        try {
            ...
            terminal.warning("Error: ${e.cause}")
            terminal.confirm("Continue?", abort = true)
        }
        ...
        terminal.info("fetched repo details: $repo")
        if (repo.parent == null) {
        ...
        terminal.info("cloning $appRepoFork in to $pachliForkRoot")
        ensureRepo(terminal, appRepoFork.gitUrl, pachliForkRoot)

        val fdroidForkRoot = File(workRoot, "fdroiddata")
        terminal.info("cloning $fdroidRepoFork in to $fdroidForkRoot")
        ensureRepo(terminal, fdroidRepoFork.gitUrl, fdroidForkRoot)

        terminal.success("Cloned repos")
        val config = Config(
